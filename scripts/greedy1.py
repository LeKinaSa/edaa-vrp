...
def generate_new_journey(destinations, data):
    ...
    while True:
        ...
        for location, cost in list(enumerate(data['distance_matrix'][journey[-1]]))[1:]:
            if destinations[location - 1] and current_capacity + data['demands'][location] <= data['vehicle_capacity'] and (cost / data['demands'][location]) <= best_cost:
                best_cost = (cost / data['demands'][location])
